...
import com.yc.vo.standard.TransverseHisPage;
...
/**
 * 横向申请详情;(transverse_apply_info)表数据库访问层
 *
 * @author : http://www.chiner.pro
 * @date : 2023-10-17
 */
@Mapper
public interface TransverseApplyInfoMapper extends BaseMapper<TransverseApplyInfo> {
    /**
     * 分页查询指定行数据
     *
     * @param page    分页参数
     ...
     * @return 分页对象列表
     */
    IPage<TransverseApplyInfo> selectByPage(IPage<TransverseApplyInfo> page, @Param(Constants.WRAPPER) Wrapper<TransverseApplyInfo> wrapper);
     ...
     **/
    List<TransverseHisPage> queryPaperHisPage(@Param("begin") Long begin, @Param("size") Long size,
     ...
     **/
    int insertBatch(@Param("list") List<TransverseApplyInfo> list,@Param("user") int user);
