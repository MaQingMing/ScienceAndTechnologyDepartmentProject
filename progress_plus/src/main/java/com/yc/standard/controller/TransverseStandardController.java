...
@Api(tags = "横向项目对象功能接口")
@RestController
@RequestMapping("/transverseStandard")
public class TransverseStandardController {
    @Autowired
    ...
    // 表名 主键名
    private String tableName = "transverse_standard";
    private String idName = "tsid";


    @ApiOperation("添加横向项目")
    @PostMapping("/add")
    public Result addTransverse(@RequestBody EditProject editProject, @SessionAttribute("governuser") Map governuser) {
        editProject.setCreateBy(governuser.get("id") + "");
        editProject.setUpdateBy(governuser.get("id") + "");
        transverseStandardService.addTransverse(editProject);
        return Result.success("添加成功!");
    }

    @ApiOperation("查询横向项目")
    @GetMapping("/get/{pageNum}/{pageSize}")
    public Result selectTransverse(@ApiParam(name = "pageNum", value = "当前页码")
                                   @PathVariable("pageNum") Integer pageNum,
                                   @ApiParam(name = "pageSize", value = "当前查询条数")
                                   @PathVariable("pageSize") Integer pageSize,
                                   @RequestParam(value = "context", required = false) String context,
                                   @RequestParam(value = "leid", required = false) String leid) {
        Page<EditProject> page = transverseStandardService.selectTransverse(pageNum, pageSize, context, leid);
    ...
    @ApiOperation("修改横向项目")
    @PostMapping("/update")
    public Result updateTransverse(@RequestBody EditProject editProject, @SessionAttribute("governuser") Map governuser) {
        editProject.setCreateBy(governuser.get("id") + "");
        editProject.setUpdateBy(governuser.get("id") + "");
        int result = transverseStandardService.updateTransverse(editProject);
    ...
    public Result modifyCash(@ApiParam(name = "id", value = "项目id")
        ...
        techResultsLevelService.modifyCash(tableName, cash, idName, id);
        return Result.success("修改成功!");
    }
    ...
    public Result modifyProjectStatus(@ApiParam(name = "id", value = "项目id")
        ...
        techResultsLevelService.modifyProjectStatus(tableName, status, idName,id);
        return Result.success("修改成功!");
    }
    ...
    public Result queryLevels(){
        return Result.success(transverseStandardMapper.queryLevels());
    }

    @ApiOperation("查询横向项目级别对应的分数")
    @GetMapping("/level")
    public Result selectLevel(){
        List<EditProject> results = transverseStandardService.selectLevel();
